...
\begin{document}
	...
	In order to generate the NDSI as described in Section~\ref{seq:motion_ndsi_functional}, we initialise a new numpy n-dimensional array based of the raw data of the image NDSI\(time=t+dt\), therefore making sure that the two images have the same shape and size.
	
	\begin{lstlisting}[caption={for loop motion image generation},label={lst:forloop},language=Python]
		def __create_image(self):		
			for y in range(self.__width, self.__height):
				for x in range(self.__width, self.__height):
