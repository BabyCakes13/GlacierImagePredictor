...
\begin{document}
	...
	\section{Alignment}
	Landsat's trajectory orbiting Earth is not fully precise, therefore not all images will be pixel-to-pixel aligned, which is a problem for image processing. Since we want to track each pixel's movement, we must be sure that its coordinates in the image do not change between any two given scenes. Figure~\ref{fig:unaligned_scenes} and Figure~\ref{fig:unaligned_scene_small} highlight the discrepancy from scene TODO to scene TODO, as an example.
	\begin{figure}[h]
	\centering
	\begin{minipage}{0.36\textwidth}
		\centering
		\includegraphics[width=\linewidth]{../images/unaligned.png}
		\label{fig:unaligned_scene_big}
	\end{minipage}
	\begin{minipage}{0.45\textwidth}
		\centering
		\includegraphics[width=\linewidth]{../images/unaligned_part.png}
		\label{fig:unaligned_scene_small}
	\end{minipage}
